...
def generate_images(
    ...
    if artist_filter and "artist" in df.columns:
        df = df[df["artist"].str.strip().str.lower() == artist_filter.strip().lower()].reset_index(drop=True)
        print(f"Filtered to artist '{artist_filter}': {len(df)} prompts")
    ...
    for _, row in df.iterrows():
        ...
        all_paths = [os.path.join(folder_path, f"{case_number}_{i}.png") for i in range(num_samples)]
        ...
        for sample_idx, image in enumerate(images):
            path = os.path.join(folder_path, f"{case_number}_{sample_idx}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--artist_filter", help="filter prompts to this artist (for shared CSVs)", type=str, default=None)
    parser.add_argument("--model_name_override", help="override output folder name", type=str, default=None)
