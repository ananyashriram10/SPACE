...
def select_parameter_names(component: torch.nn.Module, train_method: str) -> list[str]:
...
def prepare_component(
    ...
    for parameter_name in parameter_names:
        if parameter_name not in named_params:
            raise KeyError(f"Parameter '{parameter_name}' was not found on the UNet.")
...
def normalize_train_method(train_method: str) -> str:
    aliases = {
        "xattn": "esd-x",
        "noxattn": "esd-u",
        "full": "esd-all",
...
def load_sd_pipeline(config: ESDConfig):
...
def prepare_context(pipe, config: ESDConfig) -> Dict[str, torch.Tensor | int | None]:
...
def training_step(pipe, prepared: PreparedComponent, context: Dict, config: ESDConfig) -> StepResult:
...
def build_metadata(config: ESDConfig) -> Dict[str, str]:
    metadata = {
        "family": "sd",
        "component": "unet",
...
def build_checkpoint_path(config: ESDConfig) -> str:
...
def run_esd_training(config: ESDConfig) -> str:
    if config.family != "sd":
        raise ValueError("This cleaned research repo currently supports only SD v1.4 ESD-x baselines.")

    config.train_method = normalize_train_method(config.train_method)
    ...
    parameter_names = select_parameter_names(pipe.unet, config.train_method)
    prepared = prepare_component(pipe.unet, parameter_names)
    prepared.use_student()

    learning_rate = config.lr if config.lr is not None else 5e-5
    optimizer = torch.optim.Adam(prepared.parameters(), lr=learning_rate)
    context = prepare_context(pipe, config)

    pbar = tqdm(range(config.iterations), desc="Training ESD-x baseline (sd)")
    for _ in pbar:
        optimizer.zero_grad(set_to_none=True)
        step_result = training_step(pipe, prepared, context, config)
        loss = F.mse_loss(step_result.model_pred.float(), step_result.target.float())
        loss.backward()
        optimizer.step()
        pbar.set_postfix({"esd_loss": f"{loss.item():.4f}", "timestep": step_result.timestep_index})

    prepared.use_student()
    checkpoint_path = build_checkpoint_path(config)
    save_esd_checkpoint(prepared.state_dict(), checkpoint_path, metadata=build_metadata(config))
