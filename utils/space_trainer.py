...
def make_space_teacher(
    ...
    neutral_anchor = (1.0 - config.alpha_art) * base_content + config.alpha_art * base_art
    style_direction = base_target - base_content
    return neutral_anchor - config.erase_scale * style_direction
...
def ranked_saliency_modules(pipe, families, null_embeds, timestep_cond, resolution, config) -> list[str]:
    candidate_names = select_cross_attention_linears(pipe.unet)
    if not candidate_names:
        raise ValueError("No cross-attention linear modules found for SPACE saliency selection.")
    ...
    for family in probe_families:
        ...
        for idx in band_indices:
            ...
            with torch.no_grad():
                ...
                teacher = make_space_teacher(base_target, base_content, base_art, config)
...
def record_space_provenance(config: SPACEConfig, checkpoint_path: str, selected_modules: list[str]) -> None:
    out_dir = os.path.join("results", "provenance", "space")
...
def run_space_training(config: SPACEConfig) -> str:
    ...
    optimizer = torch.optim.Adam(list(lora_parameters(adapters)), lr=config.lr)
    ...
    pbar = tqdm(range(total_steps), desc="Training SPACE v1 (sd)")
    ...
    for step in pbar:
        ...
        for band_idx in trajectory:
            ...
            if general_preserve_contexts and config.lambda_preserve > 0:
