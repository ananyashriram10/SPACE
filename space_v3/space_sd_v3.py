...
# Ensure erasing/ (parent of space_v3/) is on sys.path so utils.* imports resolve
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="TrainSPACE-v3 for SD",
        description=(
            "Train SPACE-v3 for SD v1.4: a preservation-first dual-anchor artist/style "
            "erasure method tuned to preserve better than UCE while matching SPACE-v1/UCE erasure."
        ),
    )
    parser.add_argument(
        "--basemodel_id",
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.25,0.40,0.55",
    )
    parser.add_argument(
        "--lora_target_layers",
        help=(
            "comma-separated layer name substrings for LoRA injection. "
            "E.g. 'attn2' (v3 default), 'attn2,attn1', or 'attn2,attn1,ff' for audits."
        ),
        type=str,
        default="attn2",
    )
    ...
    parser.add_argument("--stage1_steps", help="number of stage-1 optimization steps", type=int, default=350)
    parser.add_argument("--stage2_steps", help="number of stage-2 optimization steps", type=int, default=150)
    parser.add_argument("--lr", help="learning rate", type=float, default=7e-5)
    ...
    parser.add_argument(
        "--alpha_img",
        help="blend amount toward the neutral image anchor inside the erase target",
        type=float,
        default=0.25,
    )
    parser.add_argument(
        "--erase_scale",
        help="negative style-direction strength for SPACE-v3 erasure",
        type=float,
        default=1.20,
    )
    parser.add_argument(
        "--residual_mix",
        help="blend weight for content residual versus null residual in the SPACE-v3 teacher",
        type=float,
        default=0.90,
    )
    parser.add_argument("--style_gate_mode", type=str, default="residual_norm", choices=["off", "residual_norm"])
    parser.add_argument("--style_gate_min", type=float, default=0.50)
    parser.add_argument("--style_gate_max", type=float, default=1.10)
    parser.add_argument("--lambda_style", type=float, default=0.8)
    parser.add_argument("--lambda_content", type=float, default=1.25)
    parser.add_argument("--lambda_image", type=float, default=1.0)
    parser.add_argument("--lambda_preserve", type=float, default=0.75)
    parser.add_argument("--lambda_fid_preserve", type=float, default=2.0)
    parser.add_argument("--lambda_vulnerable", type=float, default=0.50)
    parser.add_argument("--lambda_lora", type=float, default=1e-4)
    parser.add_argument("--preserve_null_interval", type=int, default=10)
    parser.add_argument("--disable_preserve_null_projection", action="store_true")
    parser.add_argument(
        "--preserve_limit",
    ...
    parser.add_argument(
        "--save_path",
        help="directory to save SPACE-v3 checkpoints",
        type=str,
        default="space-models/sd-v3/",
    )
...
def main() -> None:
    ...
    from space_trainer_v3 import SPACEConfig, run_space_training
    ...
    config = SPACEConfig(
        ...
        style_gate_mode=args.style_gate_mode,
        style_gate_min=args.style_gate_min,
        style_gate_max=args.style_gate_max,
        lambda_style=args.lambda_style,
        lambda_content=args.lambda_content,
        lambda_image=args.lambda_image,
        lambda_preserve=args.lambda_preserve,
        lambda_fid_preserve=args.lambda_fid_preserve,
        lambda_vulnerable=args.lambda_vulnerable,
        lambda_lora=args.lambda_lora,
        preserve_null_interval=args.preserve_null_interval,
        preserve_null_projection=not args.disable_preserve_null_projection,
    ...
    print(f"Saved SPACE-v3 checkpoint to {checkpoint_path}")
