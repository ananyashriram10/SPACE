    ...
    for family in probe_families:
        ...
        for idx in band_indices:
            ...
            with torch.no_grad():
                ...
                teacher = make_space_teacher(base_target, base_content, base_art, base_null, config)
...
def record_space_provenance(config: SPACEConfig, checkpoint_path: str, selected_modules: list[str]) -> None:
    out_dir = os.path.join("results", "provenance", "space_v2r")
...
def run_space_training(config: SPACEConfig) -> str:
    ...
    pbar = tqdm(range(total_steps), desc="Training SPACE v2R (sd)")
