#!/bin/bash
# Generate SPACE-v2R images for one Van Gogh candidate.
...
WEIGHTS_DIR="${WEIGHTS_DIR:-space-models/sd-v2r}"
OUT_DIR="${OUT_DIR:-results/space_v2r_e175}"
EXP_NAME="${EXP_NAME:-Van_Gogh_v2r_e175}"
...
if [ "$ONLY_ARTIST" != "Van Gogh" ]; then
  echo "SPACE-v2R image generation is currently scoped to Van Gogh only. Received ONLY_ARTIST=$ONLY_ARTIST"
  exit 1
fi

CKPT="$ROOT/$WEIGHTS_DIR/space-$EXP_NAME.safetensors"
if [ ! -f "$CKPT" ]; then
  echo "Missing SPACE-v2R checkpoint: $CKPT"
  echo "Run space_v2/run_space_v2r_sweep.sh or train this EXP_NAME first."
  exit 1
fi

echo "============================================================"
echo "  SPACE-v2R Image Generation"
echo "============================================================"
...
"$PYTHON_BIN" "$ROOT/evalscripts/generate-images.py" \
  ...
  2>&1 | tee "$ROOT/logs/space_v2r_images_${EXP_NAME,,}.log"

mkdir -p "$ROOT/results/provenance/space_v2r"
...
payload = {
    ...
    "checkpoint": f"space-models/sd-v2r/space-{exp_name}.safetensors",
    "outputs": [f"{out_dir}/{model_name}"],
    "prompts": "data/vangogh_prompts.csv",
}
out = root / "results" / "provenance" / "space_v2r" / f"images_space-{exp_name}.json"
out.write_text(json.dumps(payload, indent=2) + "\n")
print(f"Wrote provenance: {out}")
PY

echo ""
echo "SPACE-v2R image generation complete."
